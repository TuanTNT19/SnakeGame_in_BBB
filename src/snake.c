...
//Display game over
void Snake_gameOverScreen(int fd) {
    OLED_SetCursor(fd, 25, 3);
    OLED_Display(fd, "GAME OVER!");
    OLED_SetCursor(fd, 10, 4);
    OLED_Display(fd, "PRESS TO CONTINUE");
    KEYBOARD_waitForAnyKey();
}


void Snake_startGame(int fd, int snakeXY[][SNAKE_ARRAY_SIZE], int foodXY[], int ScreenWidth, int ScreenHeight, int snakeLength, int direction, int score, int speed) {
    int gameOver = 0;
    clock_t endWait;
    ...
    do {
        if (canChangeDirection) {
            ...
            direction = KEYBOARD_KeyPress_to_Direction(direction);
        }
        ...
        {
            Snake_move(fd, snakeXY, snakeLength, direction);
            canChangeDirection = 1;

            if (Snake_eatFood(snakeXY, foodXY)) {
                Snake_generateFood(fd, foodXY, ScreenWidth, ScreenHeight, snakeXY, snakeLength); //Generate More Food
                snakeLength++;
                score += 10;
                ...
                Snake_refreshInfoBar(fd, score, speed);
            }
...
void Snake_loadGame(int fd) {
    ...
    Snake_loadSnake(fd, snakeXY, snakeLength);
    Snake_generateFood(fd, foodXY, ScreenWidth, ScreenHeight, snakeXY, snakeLength);
    Snake_refreshInfoBar(fd, score, speed); //Bottom info bar. Score, Level etc
    Snake_startGame(fd, snakeXY, foodXY, ScreenWidth, ScreenHeight, snakeLength, direction, score, speed);
